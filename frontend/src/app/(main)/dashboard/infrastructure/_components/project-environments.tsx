          ...
          <TableRow>
            <TableHead className="font-medium">
              <span className="inline-flex items-center gap-1">
                Source <ArrowUpDown className="size-4" />
              </span>
            </TableHead>
        ...
        <TableBody className="**:data-[slot='table-row']:hover:bg-transparent">
          {rows.map((row) => (
            <TableRow key={row.source}>
              <TableCell>
                <span className="block truncate font-medium" title={row.source}>
                  {row.source}
                </span>
              </TableCell>
              <TableCell>
                <span className="flex items-center gap-2 font-medium text-muted-foreground">
                  <SimpleIcon icon={row.adapter.icon} className="size-4 fill-current" />
                  {row.adapter.name}
                </span>
              </TableCell>
              <TableCell>
                <Badge
                  variant={row.mode === "Planned" ? "destructive" : "secondary"}
                  className={cn(
                    "rounded-sm px-1.5 py-0.5",
                    row.mode === "Writable" && "bg-emerald-500/10 text-emerald-600 dark:text-emerald-400",
                    row.mode === "Read-only" && "bg-sky-500/10 text-sky-600 dark:text-sky-400",
                  )}
                >
                  {row.mode}
                </Badge>
              ...
              <TableCell>
                <Badge
                  variant={row.health === "Synced" ? "secondary" : "destructive"}
                  className={cn(
                    "rounded-sm px-1.5 py-0.5",
                    row.health === "Synced" && "bg-emerald-500/10 text-emerald-600 dark:text-emerald-400",
                    row.health === "Lagging" && "bg-amber-500/10 text-amber-600 dark:text-amber-400",
                  )}
                >
                  <span
                    className={cn(
                      "size-1.5 rounded-full",
                      row.health === "Synced" ? "bg-emerald-500" : row.health === "Lagging" ? "bg-amber-500" : "bg-destructive",
                    )}
                  />
                  {row.health}
                </Badge>
              ...
              <TableCell>
                <div className="grid grid-cols-3 gap-4">
                  <ResourceMeter label="Recall" value={row.resources.recall} />
                  <ResourceMeter label="Arch" value={row.resources.architecture} />
                  <ResourceMeter label="Write" value={row.resources.write} />
                </div>
              </TableCell>
              <TableCell>
  ...
  return (
    <div className="flex min-h-24 items-center justify-center border-t bg-muted/50 p-4">
      <div className="flex items-center gap-2">
        ...
        <p className="font-medium text-sm">No context sources connected to this project</p>
      </div>
    </div>
  );
}
