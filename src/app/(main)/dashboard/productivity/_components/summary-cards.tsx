...
const iconsByTitle = {
  ...
  "Pending handoffs": Waypoints,
  "Writable repos": GitPullRequest,
} as const;

export function SummaryCards({ items }: { items: Array<{ description: string; title: string; value: string }> }) {
  return (
    <div className="grid gap-4 md:grid-cols-3">
      {items.map((item) => {
        const Icon = iconsByTitle[item.title as keyof typeof iconsByTitle] ?? FolderKanban;
