  ...
  const tokenRoles: Role[] = projects.flatMap((project) =>
    project.accessTokens.map((token) => {
      ...
      const status: Role["status"] =
        token.revokedAt || broadRepoScope || daysToExpiry <= 14 ? "Needs review" : "Active";

      return {
        role: token.label,
        group: "Write tokens",
        accessLevel: token.scopes.includes("write") || token.scopes.includes("repo") ? "Scoped write" : "Read only",
        users: 1,
